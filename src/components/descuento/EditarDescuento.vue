  ...
  methods: {
    ...
    verificarDisponibilidad() {
      ...
      if (
        ...
        this.nombreOriginal != this.descuento.motivo
      ) {
        this.loading = true;
        axios
          .get(`${process.env.BASE_URL}/api/cliente/existe-descuento`, {
            params: {
          ...
          .then(res => {
            ...
            if (res.data.existe == true) {
              ...
              this.errorDisponibilidad =
                "Ya existe un descuento registrado con ese motivo.";
            } else {
    ...
    modificarDescuento() {
      ...
      if (this.checkForm()) {
        ...
        axios
          .post(
            `${process.env.BASE_URL}/api/cliente/modificar-descuento`,
            params
          )
          .then(res => {
            console.log(res);
            if (res.data.resultado == 5450) {
              this.$router.push({
                name: "PrincipalPlan",
                params: {
                  resultadoOperacion: "Descuento modificado satisfactoriamente."
                }
              });
              this.limpiarCajas();
    ...
    checkForm() {
      ...
      if (!this.descuento.porcentaje) {
        ...
        this.erroresForm.push(
          "El valor del porcentaje debe ser un número entre 0 y 100."
        );
      }
