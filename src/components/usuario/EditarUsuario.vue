...
export default {
  ...
  mounted() {
    this.loading = true;
    axios
      .get(`${process.env.BASE_URL}/api/tipo-usuario/lista-tipos`)
      .then(res => {
        console.log(res);
  ...
  methods: {
    ...
    verificarDisponibilidad() {
      if (
        this.usuario.nombre != "" &&
        this.usuario.nombre != this.nombreOriginal
      ) {
        axios
          .get(`${process.env.BASE_URL}/api/usuario/existe-usuario`, {
            params: {
          ...
          .then(res => {
            ...
            if (res.data.existe == true) {
              ...
              this.errorDisponibilidad =
                "Nombre de usuario ya se encuentra en uso.";
            } else {
    ...
    modificarUsuario() {
      if (this.checkForm()) {
        ...
        console.log(params);
        axios
          .post(`${process.env.BASE_URL}/api/usuario/modificar-usuario`, params)
          .then(res => {
            console.log(res);
            if (res.data.resultado == 1100) {
              this.$router.push({
                ...
                params: {
                  resultadoOperacion: "Usuario modificado satisfactoriamente."
                }
              });
              this.limpiarCajas();
              ...
              this.resultadoOperacion =
                "Ha ocurrido un error, por favor comuníquese con el soporte.";
            }
          })
          .catch(error => {
            this.alerta = true;
    ...
    checkForm() {
      ...
      if (
        ...
        nombreValido &&
        (!this.usuario.contrasena ||
          (this.usuario.contrasena && contrasenaValida)) &&
        this.usuario.tipo.id != 0
      ) {
        return true;
      }
      ...
      if (!this.usuario.nombre) {
        ...
        this.erroresForm.push(
          "Evite ingresar caracteres que no sean letras en el nombre."
        );
      }
      if (this.usuario.contrasena && !contrasenaValida) {
        this.erroresForm.push(
          'La contraseña debe contener entre 8 y 16 caracteres: al menos una letra mayúscula, una minúscula y un número. Puede contener caracteres especiales como "?", "@", "." y "-".'
        );
      }
